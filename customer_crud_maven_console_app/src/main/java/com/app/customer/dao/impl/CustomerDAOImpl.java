...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public int updateCustomer(int id, long contact) throws BusinessException {
		...
		try(Connection connection=MySqlConnection.getConnection())
		{
			...
			if(c==0)
			{
				throw new BusinessException("Customer was not updated, is the customer id correct?");
			}
		}
		catch (ClassNotFoundException | SQLException e)
		{
	...
	public int deleteCustomer(int id) throws BusinessException {
		...
		try(Connection connection=MySqlConnection.getConnection())
		{
			...
			if(c==0)
			{
				throw new BusinessException("Customer was not deleted, is the customer id correct?");
			}
		}
		catch (ClassNotFoundException | SQLException e)
		{
	...
	public Customer getCustomerById(int id) throws BusinessException {
		...
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,gender,city,age from customer where id=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
			preparedStatement.setLong(1, id);
	...
	public List<Customer> getCustomersByAge(int age) throws BusinessException {
		...
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,gender,city,age from customer where age=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
			...
			if(resultSet.next()) {
				...
				throw new BusinessException("No customers with the age of "+age+" in the DB");
			}
		}catch (ClassNotFoundException | SQLException e) {
			System.out.println(e); //for test
			throw new BusinessException("Internal error occured... Kindly contact SYSADMIN!!!!!....");
	...
	public List<Customer> getCustomersByName(String name) throws BusinessException {
		List<Customer> customerList= new ArrayList<>();
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,gender,city,age from customer where name=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
	...
	public List<Customer> getCustomersByCity(String city) throws BusinessException {
		List<Customer> customerList= new ArrayList<>();
		try(Connection connection=MySqlConnection.getConnection()){
			String sql="select id,name,gender,city,age from customer where city=?";
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
